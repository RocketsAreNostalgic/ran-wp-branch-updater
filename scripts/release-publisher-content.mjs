...
function firstNotes(changelog, version) {
  const escaped = version.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); const date = "\\([0-9]{4}-[0-9]{2}-[0-9]{2}\\)"; const heading = new RegExp(`^## (?:${escaped} ${date}|\\[${escaped}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-wp-branch-updater/compare/v[^)\\s]+\\.\\.\\.v${escaped}\\) ${date})\\n`, "m");
...
export function candidateIdentity(contents, candidateSha) {
  ...
  if (composer?.name !== "ran/wp-branch-updater" || composer?.type !== "library" || Object.hasOwn(composer, "version")) refuse("composer_identity_invalid", "Composer identity is not the unversioned target library");
...
export function verifyReleaseDelta(parent, candidate) {
  ...
  const parentHeading = before === UNRELEASED
    ? /^## \[Unreleased\]\n/m
    : new RegExp(`^## (?:${escapedBefore} ${date}|\\[${escapedBefore}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-wp-branch-updater/compare/v[^)\\s]+\\.\\.\\.v${escapedBefore}\\) ${date})\\n`, "m");
  const candidateHeading = before === UNRELEASED
    ? new RegExp(`^## ${escapedAfter} ${date}\\n`, "m")
    : new RegExp(`^## \\[${escapedAfter}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-wp-branch-updater/compare/v${escapedBefore}\\.\\.\\.v${escapedAfter}\\) ${date}\\n`, "m");
